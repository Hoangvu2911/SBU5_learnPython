...
import type { Seat, SeatMapResponse } from "./type.ts";
...
const titleEl = requireEl("seat-title", HTMLHeadingElement);
const subEl = requireEl("seat-sub", HTMLParagraphElement);
const grid = requireEl("seat-grid", HTMLDivElement);
const errEl = requireEl("seat-error", HTMLParagraphElement);
...
function renderSeats(seats: Seat[]): void {
    grid.innerHTML = "";
...
async function fetchAndRender(): Promise<void> {
    ...
    try {
        errEl.hidden = true;
        const data = await api<SeatMapResponse>(`/api/showtimes/${showtimeId}/seats/`);
        ...
        errEl.hidden = false;
        errEl.textContent = err instanceof Error ? err.message : "Lỗi khi tải ghế";
    } finally {
        inFlight = false;
    }
}
